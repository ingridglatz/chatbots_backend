...
const server = app.listen(PORT, () => {
  logger.info(
    `🚀 ChatBots API rodando na porta ${PORT} [${process.env.NODE_ENV || "development"}]`,
  );
  ...
  setTimeout(async () => {
    try {
      const {
        initializeAllBots,
      } = require("./src/services/whatsapp/whatsappManager");
      await initializeAllBots();
    } catch (err) {
      logger.warn("WhatsApp: erro ao inicializar bots (não crítico)", {
        error: err.message,
      });
    }
  }, 3000);
